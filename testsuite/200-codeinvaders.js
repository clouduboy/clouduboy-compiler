...
///      game.drawImage(gfxInvader, 13*x+4-Math.abs(game.frameCount%30/5-3), 9*y);
    let d = game.frameCount%60<30 ? Math.floor(game.frameCount%60/10) : (3 - Math.floor(game.frameCount%60/10));

    game.drawImage(Math.floor(game.frameCount/10)%2 ? gfxInvader : gfxInvader2, 13*x +(y%2 ? d : 3-d) +3, 9*y);
  ...
  game.drawText( Math.floor(game.frameCount%30/5)-3, 100,0);
