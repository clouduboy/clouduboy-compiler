...
PROGMEM const unsigned char gfx_invader[] = {
  /*9x6*/ 0x38, 0x1c, 0x35, 0x1e, 0x3c, 0x1e, 0x35, 0x1c, 0x38 };
PROGMEM const unsigned char gfx_invader_2[] = {
  /*9x6*/ 0x18, 0x3c, 0x15, 0x3e, 0x1c, 0x3e, 0x15, 0x3c, 0x18 };
...
while (y < 4) {
  int x = 0;
  while (x < 5) {
  int d = (_microcanvas_frame_counter % 60 < 30 ? floor( _microcanvas_frame_counter % 60 / 10 ) : 3 - floor( _microcanvas_frame_counter % 60 / 10 ));
  arduboy.drawBitmap( (13 * x + (y % 2 ? d : 3 - d)) + 3, 9 * y, (floor( _microcanvas_frame_counter / 10 ) % 2 ? gfx_invader : gfx_invader_2), (floor( _microcanvas_frame_counter / 10 ) % 2 ? GFX_INVADER_WIDTH : GFX_INVADER_2_WIDTH), (floor( _microcanvas_frame_counter / 10 ) % 2 ? GFX_INVADER_HEIGHT : GFX_INVADER_2_HEIGHT), WHITE );
  x = x + 1;
}
  y = y + 1;
}
...
arduboy.print( floor( _microcanvas_frame_counter % 30 / 5 ) - 3 );
};
